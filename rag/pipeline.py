...
answer layer is the local deterministic builder in rag/answer.py; there is no
model call and no credential anywhere in the pipeline.
...
class RagPipeline:
    ...
    def answer(self, question, k=None, qtype=None):
        ...
        ans = construct_answer(question, retrieved, policy=self.policy, qtype=qtype)
        return {
            "question": question,
            "answer": ans["text"],
            "refused": ans["refused"],
...
if __name__ == "__main__":
    import sys
    p = RagPipeline()
    q = " ".join(sys.argv[1:]) or "What rights does Article 31 provide?"
    out = p.answer(q)
